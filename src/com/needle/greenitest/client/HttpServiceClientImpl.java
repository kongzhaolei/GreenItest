...
import com.needle.greenitest.dto.Constant;
import com.needle.greenitest.util.FileUtil;
...
public class HttpServiceClientImpl implements ServiceInterfaceClient {
	...
	public Object invokeServiceMethod(CaseData casedata, Config config) {
		...
		if (casedata.getInputFile().getName().endsWith(Constant.FILE_TYPE_TPL)) {
			this.casedata.setInputFile(this.casedata.getVarGen()
					.processTemplate(this.casedata.getInputFile()));
			this.casedata.setInput();
		}

		String response = "";

		if (casedata.getRequesttype().equalsIgnoreCase(
		...
		// 把response记录到output中
		File resfile = FileUtil.rewriteFile(casedata.getCaselocation()
				+ Constant.FILENAME_OUTPUT, casedata.getInputFile().getName()
				.substring(0, casedata.getInputFile().getName().indexOf("."))
				+ ".response", response);
		// 调用赋值
	...
	private String requestPostMethod() {
		...
		if (config.getVariable() != null
				&& config.getVariable().containsKey(
						Constant.V_CONFIG_VARIABLE_COOKIE)) {
			postMethod.addRequestHeader(
					Constant.V_CONFIG_VARIABLE_COOKIE,
					(String) config.getVariable().get(
							Constant.V_CONFIG_VARIABLE_COOKIE));
		...
		if (parammap.size() == 1
				&& (parammap.containsKey("params") || parammap
						.containsKey("Params"))) {
			String key = "";
			...
			for (Map.Entry<String, Object> entry : parammap.entrySet()) {
				log.info("[HTTP post params]" + (String) entry.getKey() + ":"
		...
		try {
			...
			// 处理内容
			log.info("[The Post Request's Response][" + casedata.getAction()
					+ "]" + respStr);
			...
			// 请求都异常了，判定case fail
			throw new AssertionFailedError("HTTP REQUEST ERROR:"
					+ e.getMessage());
		} finally {
			// 释放连接
	...
	private String requestGetMethod() {
		...
		try {
			...
			respStr = new String(responseBody, "UTF-8");
			// 处理内容
			log.info("[The Get Request's Response][" + casedata.getAction()
					+ "]" + respStr);
